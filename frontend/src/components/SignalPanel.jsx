...
const SignalPanel = () => {
  ...
  // Real-time MTF analysis — no backend needed
  const { mtfBias, signal: liveSignal, loading } = useLiveMTF(symbol, timeframe);

  // Push live signal into store when it arrives
  useEffect(() => {
    if (liveSignal) setActiveSignal(liveSignal);
  }, [liveSignal]); // eslint-disable-line
    ...
    <div className="w-70 border-l border-[var(--border)] flex flex-col overflow-y-auto" style={{ backgroundColor: 'var(--bg-secondary)' }}>
      ...
      {signal ? (
        <div data-testid="active-signal-card" className="m-3 rounded-lg overflow-hidden" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
          ...
          <div className="p-3">
            ...
            <div className="text-xs mb-3" style={{ color: 'var(--text-secondary)' }}>{signal.session || 'Live Analysis'}</div>
            ...
            <div className="space-y-2 mb-4">
              {[
                { label: 'Entry',  value: signal.entry,      color: 'var(--text-primary)' },
                { label: 'Stop',   value: signal.stop_loss,  color: 'var(--accent-red)' },
                { label: 'Target', value: signal.take_profit, color: 'var(--accent-green)' },
              ].map(({ label, value, color }) => (
                <div key={label} className="flex items-center justify-between text-sm">
                  <span style={{ color: 'var(--text-secondary)' }}>{label}</span>
                  <div className="flex items-center gap-2">
                    <span className="monospace font-semibold" style={{ color }}>${(value || 0).toFixed(2)}</span>
                    <button onClick={() => copyToClipboard((value || 0).toString())} className="p-1 rounded hover:bg-[var(--bg-hover)]">
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="monospace font-semibold" style={{ color: 'var(--text-primary)' }}>1 : {(signal.risk_reward || 2).toFixed(1)}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button className="flex-1 py-2 text-sm font-medium rounded" style={{ backgroundColor: 'var(--accent-green)', color: 'white' }}>
                ✓ TAKE TRADE
              </button>
              <button className="flex-1 py-2 text-sm font-medium rounded flex items-center justify-center gap-1" style={{ backgroundColor: 'var(--bg-hover)', color: 'var(--text-primary)' }}>
        ...
        <div data-testid="scanning-state" className="m-3 p-8 text-center rounded-lg" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
          ...
          <div className="text-sm font-medium mb-1" style={{ color: 'var(--text-primary)' }}>
            {loading ? 'ANALYZING MTF...' : 'WAITING FOR SETUP'}
          </div>
          <div className="text-xs" style={{ color: 'var(--text-secondary)' }}>
            {loading ? `${symbol} across 4H / 1H / 15M / 5M` : 'No high-confidence confluence yet'}
          </div>
        </div>
      )}

      {/* MTF Bias — live calculated */}
      <div className="mx-3 mb-3 p-3 rounded-lg" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
        ...
        <div className="space-y-2">
          {(mtfBias.length > 0 ? mtfBias : [
            { timeframe: '4H', bias: '...', direction: 'neutral', strength: 0 },
            { timeframe: '1H', bias: '...', direction: 'neutral', strength: 0 },
            { timeframe: '15M', bias: '...', direction: 'neutral', strength: 0 },
            { timeframe: '5M', bias: '...', direction: 'neutral', strength: 0 },
          ]).map((bias) => (
      ...
      <div className="mx-3 mb-3 p-3 rounded-lg" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            { val: mockQuickStats.signals_today, label: 'Signals',  color: 'var(--text-primary)' },
            { val: `${mockQuickStats.win_rate}%`, label: 'Win %',   color: 'var(--accent-green)' },
            { val: `+${mockQuickStats.pnl_r}R`,  label: 'P&L',     color: 'var(--accent-green)' },
            { val: `${mockQuickStats.drawdown}%`, label: 'DD',      color: 'var(--accent-red)' },
          ].map(({ val, label, color }) => (
            <div key={label} className="text-center p-2 rounded" style={{ backgroundColor: 'var(--bg-secondary)' }}>
