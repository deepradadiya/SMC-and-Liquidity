...
from .smc_engine import PreciseSMCEngine
from ..models.smc_models import SMCAnalysis, SMCDetectionConfig
...
# Legacy analysis and signals routes (merged from old routes)
...
@router.post("/legacy/analyze", response_model=dict)
async def analyze_smc_patterns(request: AnalysisRequest):
    """Run Smart Money Concepts analysis on market data (legacy endpoint)"""
    try:
        from ..core.market_data_service import MarketDataService
        ...
        # Fetch market data
        market_data = await market_data_service.fetch_ohlcv(request.symbol, request.timeframe, request.limit)
        df = market_data_service.to_dataframe(market_data)
        
        # Run SMC analysis
        analysis = smc_strategy.analyze(df)
...
@router.post("/legacy/signals", response_model=dict)
async def generate_signals(request: SignalRequest):
    """Generate trading signals (legacy endpoint)"""
    try:
        from ..core.market_data_service import MarketDataService
        from ..core.signal_generator import SignalGenerator
        from ..module2_risk_manager.risk_manager import RiskManager
        ...
        # Fetch market data
        market_data = await market_data_service.fetch_ohlcv(request.symbol, request.timeframe, 500)
        df = market_data_service.to_dataframe(market_data)
        
        # Generate signals
        signals = signal_generator.generate_signals(df, request.symbol, request.timeframe)
        
        # Filter by confidence
        filtered_signals = [s for s in signals if s.confidence >= request.min_confidence]
        ...
        for signal in filtered_signals:
            ...
            if validation.approved:
                validated_signals.append({
                    "signal": signal.dict(),
                    "risk_validation": validation.dict()
                })
...
# Import datetime for legacy routes
