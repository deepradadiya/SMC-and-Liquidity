...
from ..module5_security.auth import get_current_user, UserInfo
from ..core.logger import get_logger
from ..module5_security.rate_limiter import limiter
...
# Historical Data Routes (merged from historical_data.py)
...
@router.get("/historical/timeframes", response_model=Dict[str, TimeframeInfo])
...
@router.get("/historical/fetch/{symbol}", response_model=BulkHistoricalDataResponse)
...
@router.get("/historical/fetch/{symbol}/{timeframe}", response_model=HistoricalDataResponse)
async def fetch_timeframe_data(symbol: str, timeframe: str):
...
@router.get("/historical/summary", response_model=DataSummaryResponse)
async def get_data_summary(symbol: Optional[str] = Query(None, description="Filter by symbol")):
...
# Import datetime for historical data functions
