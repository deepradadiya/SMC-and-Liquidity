...
class TimeframeHierarchy:
    """Timeframe hierarchy and validation utilities"""
    
    # Timeframe definitions
    HTF_TIMEFRAMES = ["1d", "4h", "12h"]
    MTF_TIMEFRAMES = ["1h", "2h", "4h"]
    LTF_TIMEFRAMES = ["1m", "5m", "15m", "30m"]
    
    ALL_TIMEFRAMES = HTF_TIMEFRAMES + MTF_TIMEFRAMES + LTF_TIMEFRAMES
    
    @classmethod
    def get_timeframe_type(cls, timeframe: str) -> str:
        """Get the type of timeframe (HTF, MTF, or LTF)"""
        if timeframe in cls.HTF_TIMEFRAMES:
            ...
            raise ValueError(f"Invalid timeframe: {timeframe}. Must be one of: {cls.ALL_TIMEFRAMES}")
    
    @classmethod
    def validate_hierarchy(cls, htf: str, mtf: str, ltf: str) -> bool:
        """Validate that timeframes follow proper hierarchy (HTF > MTF > LTF)"""
        try:
            htf_type = cls.get_timeframe_type(htf)
            mtf_type = cls.get_timeframe_type(mtf)
            ltf_type = cls.get_timeframe_type(ltf)
            
            # Check hierarchy order
            if htf_type != "HTF":
                raise ValueError(f"HTF must be a higher timeframe, got: {htf}")
            if mtf_type not in ["MTF", "HTF"]:
                raise ValueError(f"MTF must be a medium timeframe, got: {mtf}")
            if ltf_type != "LTF":
                raise ValueError(f"LTF must be a lower timeframe, got: {ltf}")
